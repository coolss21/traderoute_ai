...
export default function GlobeMapInner({ routes, hoveredRoute, width, height, showAllPorts, isSpinning = true }: Props) {
      ...
      for (let i = 0; i < route.path.length - 1; i++) {
        ...
        if (s && e) {
          arcs.push({
            ...
            mode: route.mode,
            routeName: `${route.path[0]} ΓåÆ ${route.path[route.path.length - 1]}`,
  ...
  // Arc color ΓÇö dim non-hovered routes
  ...
  // Arc stroke ΓÇö sea is thick & bold, air is thin
  ...
  // Same arc altitude for both ΓÇö all arcs visible above the globe
  ...
  return (
    <Globe
      ...
      // ΓöÇΓöÇ Arcs ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      arcsData={arcsData}
      ...
      arcLabel={(d: object) => {
        const arc = d as ArcDatum;
        const modeIcon = arc.mode === "sea" ? "≡ƒÜó" : "Γ£ê∩╕Å";
        ...
        return `
          ...
          ">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:6px 16px; font-size:11px;">
              ...
              <div style="color:rgba(255,255,255,0.5);">COΓéé</div>
              <div style="color:#4ade80; font-weight:600;">${arc.co2.toLocaleString()} kg</div>
            </div>
            ${arc.isRecommended ? '<div style="margin-top:8px; padding:4px 8px; background:rgba(74,222,128,0.15); border:1px solid rgba(74,222,128,0.3); border-radius:6px; font-size:10px; color:#4ade80; font-weight:700; text-align:center; text-transform:uppercase; letter-spacing:1px;">Γ¡É Recommended Route</div>' : ''}
          </div>
        `;
      }}
      // ΓöÇΓöÇ City dot markers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      pointsData={pointsData}
      pointColor={(d: object) => (d as PointDatum).isMajor ? "#f59e0b" : "#ffffff"}
      pointAltitude={0.012}
      pointRadius={0.38}
      // ΓöÇΓöÇ City labels ΓÇö raised well above arc lines ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ...
      labelAltitude={0.055}
      labelResolution={3}
      // ΓöÇΓöÇ Disruption Rings ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ringsData={(() => {
